...
export abstract class CompassControl<ModelType, T> {
    ...
    if (!c.errorMessage) {
      c.errorMessage = errors => {
        if (errors) {
          ...
          if (errorKey === "min") {
            return "Minimal value is " + errors[errorKey].min;
          }
          if (errorKey === "max") {
            return "Max value is " + errors[errorKey].max;
          }
          if (errorKey === "pattern") {
            return "Incorrect format";
          }
          if (errorKey === "maxlength") {
            return "Max length is " + errors[errorKey].requiredLength;
          }
          if (errorKey === "invalidInteger") {
            return "Value must be an integer";
          }
          if (errorKey === "IsNumeric") {
            return "Not a number";
          }
          if (errorKey === "matDatepickerMin") {
            return (
              "Date cannot be before " +
              errors[errorKey].min.format("MMM D, YYYY")
            );
          }
          if (errorKey === "matDatepickerMax") {
            return (
              "Date cannot be after " +
              errors[errorKey].max.format("MMM D, YYYY")
            );
          }
  ...
  update(model: ModelType) {
    ...
    s.required =
      c.required instanceof Function ? c.required(model) : c.required;
    ...
    s.disabled =
      c.disabled instanceof Function ? c.disabled(model) : c.disabled;

    const errors = this.ngControl.errors;
    if (errors) {
      s.errorMessage =
        c.errorMessage instanceof Function
          ? c.errorMessage(errors)
          : c.errorMessage;
    } else {
      s.errorMessage = "";
  ...
  addValidator(validator: ValidatorFn): void {
    ...
    this.ngControl.setValidators(
      existingValidator ? [existingValidator, validator] : validator
    );
  }
}
