import { Component, Input, OnInit, ComponentFactoryResolver, ViewContainerRef, ViewEncapsulation, InjectionToken, Inject, Type } from '@angular/core';
import { CompassForm } from '../compass-form';
import { CompassControl, ICompassComponent } from '../compass-control';
import { CompassFieldComponents, CompassFieldComponentClass } from '../compass-field-components';
...
export class CompassFormItemComponent<ModelType, T> implements OnInit {
    ...
    constructor(
      ...
      @Inject(CompassFieldComponents) private fieldComponents: CompassFieldComponentClass[]) {}
    ...
    ngOnInit() {
      ...
      console.log(this.fieldComponents)
        ...
        const compType = this.fieldComponents.find(c => c.type === this.control.type)
        if(compType){
          ...
          throw new Error('CompassForm: Could not find component for type: '+this.control.type)
        }
    }
}
