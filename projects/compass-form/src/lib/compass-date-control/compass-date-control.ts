...
interface CompassDateControlConfig<ModelType> extends CompassControlConfig<ModelType, moment.Moment> {
...
export class CompassDateControl<ModelType> extends CompassControl<ModelType, moment.Moment> {
  ...
  readonly type = 'date'
    ...
    update(model: ModelType) {
        ...
        const c = this.config;
        s.minDate = c.minDate instanceof Function ? c.minDate(model) : c.minDate || defaultMinDate;
