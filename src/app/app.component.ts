...
const options = [1, 2, 3, 4, 5, 6, 7, 8, 9].map(x => ({
  label: "Option " + x,
  value: x
}));
  ...
  ngOnInit() {
    this.compassForm = new CompassForm<any>({
      ...
      hiddenTest: new CompassNumberControl({
        ...
        initialValue: 150,
        hint:
          "If this value is below the value of the hidden field (100), the textarea should disapear"
      })
    });

    this.compassForm.addControl(
      "Extra",
      new CompassStringControl({
